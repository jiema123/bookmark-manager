            ...
            const response = await fetch("/api/gemini", {
                ...
                body: JSON.stringify({
                    messages: [
                        {
                            role: "system",
                            content: `# Role专业的互联网资源挖掘专家 (Web Resource Curator)
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (errorData && typeof errorData === 'object') {
                        // Try to find a meaningful error message in common fields
                        const specificMsg = (errorData as any).error?.message || (errorData as any).error || (errorData as any).message || JSON.stringify(errorData);
                        if (specificMsg) {
                            errorMsg = `API Error (${response.status}): ${specificMsg}`;
                        }
                    }
                } catch (e) {
